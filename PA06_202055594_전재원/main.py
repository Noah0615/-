...
def readPlanAndCreate():
    ...
    alg = createOptimizer(parameters, p)
    return p, alg
...
def createOptimizer(parameters, p):
    aType = parameters['aType']
    optimizers = {1: 'SteepestAscent', 2: 'FirstChoice', 3: 'Stochastic', 4: 'GradientDescent', 5: 'SimulatedAnnealing', 6: 'GA'}
    numExp = parameters.get("numExp", 1)
    if aType in [1, 2, 3]:  # SteepestAscent, FirstChoice, Stochastic
        alg = eval(optimizers[aType] + '(p, numExp, parameters["delta"], parameters["limitStuck"])')
    elif aType == 4:  # GradientDescent
        alg = eval(optimizers[aType] + '(p, numExp, parameters["alpha"], parameters["dx"])')
    elif aType == 5:  # SimulatedAnnealing
      alg = eval(optimizers[aType] + '(p, parameters["numExp"], parameters["limitEval"])')
    elif aType == 6:  # GA
        alg = eval(optimizers[aType] + '(p, numExp)')
...
def conductExperiment(p, alg):
    ...
    optimalValue = bestMinimum  # Get the optimal value from the results
    p.storeExpResult(results, optimalValue)
